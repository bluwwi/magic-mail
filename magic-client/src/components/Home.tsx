...
const Home = () => {
  const [currentAddress, setCurrentAddress] = useState<api.Address | null>(
    null,
  );
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-[var(--text-secondary)] text-sm sm:text-base">
            Temporary email inbox — receive emails without revealing your real
            address.
          </p>
        </div>
        ...
        {currentAddress &&
          currentAddress.expires_at < Math.floor(Date.now() / 1000) && (
            <div className="mt-4 p-3 bg-[var(--warning-bg)] border border-[var(--warning)]/30 rounded-lg text-sm text-[var(--warning)] animate-slide-in">
              This address has expired. New emails will not be delivered.
              Generate a new address to continue.
            </div>
          )}
